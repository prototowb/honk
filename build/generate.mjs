...
//
// Origin (two halves):
//   • Machine facts — honk-server/lib/tools.js (tool schemas) + lib/specs.js
...
import { TOOLS }           from '../honk-server/lib/tools.js';
import { PLATFORM_SPECS }  from '../honk-server/lib/specs.js';
import { MEDIA_PROVIDERS } from '../honk-server/lib/config.js';

const ROOT = join(dirname(fileURLToPath(import.meta.url)), '..');
const pkg  = JSON.parse(readFileSync(join(ROOT, 'honk-server', 'package.json'), 'utf8'));
...
function renderToolCatalog() {
  ...
  md += `\n# Honk Tool Reference\n\n`;
  md += `**${TOOLS.length} tools** · generated from \`honk-server/lib/tools.js\` + \`lib/specs.js\` · server v${pkg.version}\n`;
  md += renderToolTables('##');
  return md;
}
...
function renderMcpJson() {
  return mcpConfig({
    command: 'node',
    args: ['${CLAUDE_PLUGIN_ROOT}/honk-server/run.js'],
    env: envPlaceholders(),
  });
}
...
function renderAgentMcpConfig() {
  return mcpConfig({
    command: 'node',
    args: [join(ROOT, 'honk-server', 'run.js')],
  });
}
...
function renderClaudeDesktopConfig() {
  return mcpConfig({
    comment: [
      ...
      '  { "command": "node", "args": ["/path/to/honk-server/run.js"] }',
      '',
