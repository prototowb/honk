...
1. **CI gate (`.github/workflows/ci.yml`):** on push to `main` + every PR, runs `npm ci` → `build:check` → `npm test` → `test:smoke` on ubuntu/node 20. **Verified green on the post-merge run** (`gh run 27647914421`).
