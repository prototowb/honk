...
export function measure(str: string | null | undefined, unit: 'chars' | 'graphemes' = 'chars'): number {
  const s = str ?? '';
...
export function sliceUnits(str: string | null | undefined, max: number, unit: 'chars' | 'graphemes' = 'chars'): string {
  const s = str ?? '';
  if (unit === 'graphemes' && segmenter) {
    let out = '', n = 0;
...
export function splitIntoChunks(text: string | null | undefined, max: number, unit: 'chars' | 'graphemes' = 'chars'): string[] {
