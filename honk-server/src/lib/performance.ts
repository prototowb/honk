...
function engagementScore(metrics: Record<string, unknown>): number {
  let total = 0;
  for (const [k, v] of Object.entries(metrics)) {
    if (EXPOSURE_METRICS.has(k)) continue;
    if (typeof v === 'number') total += v;
  }
  return total;
}

function key(platform: string, postId: string): string {
  return `${platform}:${postId}`;
}

// The latest snapshot per post (a post may have been fetched more than once).
function latestPerPost(snapshots: AnalyticsSnapshot[]): AnalyticsSnapshot[] {
  const latest = new Map<string, AnalyticsSnapshot>();
  for (const s of snapshots) {
    const k = key(s.platform, s.post_id);
    const prev = latest.get(k);
    if (!prev || s.ts > prev.ts) latest.set(k, s);
  }
...
function templatesByPost(): Map<string, string[]> {
  const map = new Map<string, string[]>();
  for (const asset of listAssets()) {
    if (!asset.template) continue;
    for (const u of asset.usage) {
      if (!u.post_id) continue;
      const k = key(u.platform, u.post_id);
      const list = map.get(k) ?? [];
      if (!list.includes(asset.template)) list.push(asset.template);
      map.set(k, list);
    }
  }
  return map;
...
export interface PostPerformance {
  ...
  account: string | null;
  ts: string;
  template?: string;               // set only when exactly one template is linked
  ambiguousTemplates?: string[];   // set when a post links more than one (mixed-template carousel)
...
export function postPerformance(filter: PerformanceFilter = {}): PostPerformance[] {
...
export function templatePerformance(filter: PerformanceFilter = {}): TemplateStat[] {
  const withTemplate = postPerformance(filter).filter(
    (p): p is PostPerformance & { template: string } => !!p.template,
  );
  const groups = new Map<string, (PostPerformance & { template: string })[]>();
...
export function formatPerformance(
  ...
  const postLines = shown.map(p =>
    `  ${p.post_id} | ${p.platform}${p.account ? `/${p.account}` : ''}`
  ...
  if (posts.length > shown.length) out.push(`  … ${posts.length - shown.length} more (raise limit to see them)`);
