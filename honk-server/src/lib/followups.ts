...
export function schedule({ platform, raw, account = '', delay }: {
  ...
  if (!platform || !SUPPORTED_PLATFORMS.includes(platform)) return null;
  const postId = extractPostId(platform, raw ?? null);
...
export function list(): FollowupJob[] { return load(); }

// Jobs whose due_at has passed, oldest-due first.
export function due(now: number | Date = Date.now()): FollowupJob[] {
...
export function recordFailure(id: string): (FollowupJob & { dropped: boolean }) | null {
  const items = load();
...
interface DrainResult { processed: number; succeeded: number; failed: number; dropped: number }
...
export async function runDue({ now = Date.now(), fetch: fetchFn = fetchMetrics }: {
  now?: number | Date;
  fetch?: typeof fetchMetrics;
} = {}): Promise<DrainResult> {
  const jobs = due(now);
  const result: DrainResult = { processed: 0, succeeded: 0, failed: 0, dropped: 0 };
  for (const job of jobs) {
    result.processed++;
    try {
      await fetchFn(job.platform, job.post_id, job.account || '');
      remove(job.id);
      result.succeeded++;
    } catch {
