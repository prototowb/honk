...
import type { ValidationResult, PolicyCheckResult, PolicyConfig } from './types.js';
...
export function validate(platform: string, content: Record<string, unknown>): ValidationResult {
  ...
  if (spec.thread && Array.isArray(c[spec.thread.field])) {
    const tweets = c[spec.thread.field] as unknown[];
    if (tweets.length === 0) {
  ...
  if (spec.media) {
    ...
    const urls = cz && Array.isArray(c[cz.field]) ? c[cz.field] as unknown[] : null;
    if (urls) {
      ...
      } else if (present && !/^https?:\/\//i.test((m as string).trim())) {
  ...
  const carouselUrls  = carouselField && Array.isArray(c[carouselField]) ? c[carouselField] as unknown[] : null;
  const hasSingle     = spec.media && typeof c[spec.media.field] === 'string' && (c[spec.media.field] as string).trim() !== '';
  ...
  if (Array.isArray(c.alt_texts) && (c.alt_texts as unknown[]).length) {
    ...
    } else if ((c.alt_texts as unknown[]).length !== carouselUrls.length) {
      errors.push(`"alt_texts" has ${(c.alt_texts as unknown[]).length} entries but there are ${carouselUrls.length} images — they must match 1:1.`);
    }
  }
...
export function contentText(platform: string, content: Record<string, unknown>): string {
  ...
  const c = content || {};
  if (spec?.thread && Array.isArray(c[spec.thread.field])) {
    return (c[spec.thread.field] as unknown[]).filter(t => typeof t === 'string').join('\n');
  }
  const field = spec?.text?.field;
  return field != null && c[field] != null ? String(c[field]) : '';
}

function tokenSet(v: unknown): string[] {
...
const isWordChar = (ch: string | undefined) => ch !== undefined && /[a-z0-9_]/.test(ch);
...
export function checkPolicy(
  ...
  policy: Partial<PolicyConfig> | null | undefined,
  ...
  const notes: string[] = [];
  const p = policy || {};
  const disc = (p.disclosures || {}) as { always?: string[]; sponsored?: string[] };
  const text = contentText(platform, content).toLowerCase();
  const present = (tok: string) => containsToken(text, String(tok).toLowerCase());
...
export function formatValidation(v: ValidationResult & { notes?: string[] }): string {
  const head = v.ok
