...
import type { BrandField, BrandProfile } from './types.js';
...
export function brandSchema(brandProfile: BrandProfile | null = null): BrandField[] {
...
export function formatBrandSchema(fields: BrandField[]): string {
  const groups: { name: string; items: BrandField[] }[] = [];
  ...
  const body = groups.map((g) => {
    const lines = g.items.map((f) => {
      ...
      const cur  = f.set
        ? `\n    ↳ current: ${Array.isArray(f.current) ? (f.current as string[]).join(', ') : f.current}`
        : '';
