...
import type { TimeWindow, BestTimesResult } from './types.js';

const DAYS = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];

interface RawWindow { day: string; hour: number; score: number; note: string; source?: 'research' | 'your-data' }
...
function blend(base: RawWindow[], observed: Array<{ day: string | number; hour: number; score?: number; note?: string }>): RawWindow[] {
...
export function bestTimes({ platform, count = 3, observedWindows = null, account: _account = '' }: {
  platform?: string;
  count?: number;
  observedWindows?: Array<{ day: string | number; hour: number; score?: number; note?: string }> | null;
  account?: string;
} = {}): BestTimesResult {
  if (!platform || !PLATFORMS.includes(platform)) {
    throw new Error(`Unknown platform "${platform}". One of: ${PLATFORMS.join(', ')}.`);
  }
  const base = (WINDOWS[platform] || []).map(w => ({ ...w, source: 'research' as const }));
  const merged = blend(base, observedWindows || []);
  merged.sort((a, b) => b.score - a.score);

  const windows: TimeWindow[] = merged.slice(0, Math.max(1, count)).map(w => ({
    day:    w.day,
...
export function formatBestTimes(result: BestTimesResult): string {
