...
interface TikTokData { publish_id?: string; [key: string]: unknown }
interface TikTokResponse { data: TikTokData; error?: { code: string; message: string } }

export async function postVideo(videoUrl: string, caption: string, privacyLevel = 'SELF_ONLY', account = ''): Promise<TikTokData> {
  ...
  const initJson = await initRes.json() as TikTokResponse;
...
export async function checkStatus(publishId: string, account = ''): Promise<TikTokData> {
  const accessToken = env('TIKTOK_ACCESS_TOKEN', account)!;
