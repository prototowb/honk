...
interface BlueskyRecord { uri: string; cid: string; identifier?: string }
interface SessionResponse { accessJwt: string; did: string }

export async function post(text: string, account = ''): Promise<BlueskyRecord & { identifier: string }> {
  ...
  const { accessJwt, did } = await sessionRes.json() as SessionResponse;
  ...
  return { ...(await recordRes.json() as BlueskyRecord), identifier };
}
