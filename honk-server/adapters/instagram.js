...
async function waitForContainer(creationId, accessToken, { tries = 12, delayMs = 3000 } = {}) {
    for (let i = 0; i < tries; i++) {
        const res = await fetch(`${BASE}/${creationId}?fields=status_code&access_token=${encodeURIComponent(accessToken)}`);
        if (res.ok) {
...
export async function comment(mediaId, message, account = '') {
    ...
    if (!res.ok)
        throw new Error(`IG comment ${res.status}: ${await res.text()}`);
    return res.json();
}
export async function getProfile(account = '') {
    ...
    if (!res.ok)
        throw new Error(`IG profile ${res.status}: ${await res.text()}`);
...
export async function getMetrics(mediaId, account = '') {
    ...
    if (!res.ok)
        throw new Error(`IG insights ${res.status}: ${await res.text()}`);
