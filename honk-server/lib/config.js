...
export function report() {
    ...
    for (const [platform, spec] of Object.entries(PLATFORM_SPECS)) {
        ...
        for (const key of spec.credentials) {
            for (const name of discoverAccounts(key))
...
export function accountsOverview() {
    ...
    for (const [platform, p] of Object.entries(rep.platforms)) {
        if (p.default.configured)
            defaultPlatforms.push(platform);
        for (const a of p.accounts) {
            if (a.configured) {
                if (!credPlatforms[a.name.toLowerCase()])
                    credPlatforms[a.name.toLowerCase()] = new Set();
    ...
    for (const n of Object.keys(credPlatforms))
        named.add(n);
