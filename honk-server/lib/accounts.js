...
function load() {
    if (!existsSync(STORE())) {
        const legacy = readJsonOr(LEGACY_ACTIVE_FILE(), {});
        return { active: legacy.active || '', accounts: {} };
    }
    return readVersioned(STORE(), { active: '', accounts: {} });
}
function save(data) {
...
export function recordHandle(account, platform, info) {
    const data = load();
    const k = key(account);
    const now = new Date().toISOString();
    const rec = data.accounts[k] || { handles: {}, created_at: now, updated_at: now };
