...
export function resolveVoice(profile, opts = {}) {
    ...
    for (const f of OVERRIDE_FIELDS) {
        if (f.key === 'audience') {
            if (isSet(platformOv.audience)) {
                effective.audience = platformOv.audience;
                sources.audience = 'platform';
            }
            else if (knownAudience) {
                effective.audience = audience;
                sources.audience = 'audience';
            }
            else {
                const b = at(base, f.basePath);
                effective.audience = isSet(b) ? String(b) : '';
                sources.audience = 'base';
            }
            continue;
        }
        const segVal = segment[f.key];
        const platVal = platformOv[f.key];
        if (isSet(platVal)) {
            effective[f.key] = platVal;
            sources[f.key] = 'platform';
        }
        else if (isSet(segVal)) {
            effective[f.key] = segVal;
            sources[f.key] = 'audience';
        }
        else {
            const b = at(base, f.basePath);
            effective[f.key] = isSet(b) ? b : (f.type === 'list' ? [] : '');
            sources[f.key] = 'base';
        }
    }
...
export function clone(from = '', to) {
    ...
    if (!target)
        throw new Error('clone needs a target account name (`to`).');
    const all = load();
    const src = all[key(from)];
    if (!src)
        throw new Error(`No brand profile to clone for ${from ? `'${from}'` : 'the default account'}.`);
    if (all[key(target)])
        throw new Error(`Account '${target}' already has a profile — clear it first or pick another name.`);
