...
function adaptOne(text, platform, spec, _opts) {
...
export function adapt(text, platforms, opts = {}) {
    ...
    if (!src)
        throw new Error('content_adapt requires non-empty text.');
    ...
    for (const p of targets) {
        ...
        if (!spec) {
            variants[p] = { format: 'single', content: {}, units: 0, limit: 0, unit: 'chars', fits: false, notes: [], error: `Unknown platform: ${p}` };
            continue;
        }
