...
export function contentText(platform, content) {
    const spec = PLATFORM_SPECS[platform];
    const c = content || {};
    if (spec?.thread && Array.isArray(c[spec.thread.field])) {
...
export function checkPolicy(platform, content, policy, { sponsored = false } = {}) {
    ...
    for (const tok of tokenSet(disc.always)) {
        if (present(tok))
            notes.push(`Disclosure "${tok}" present ✓`);
        else
            warnings.push(`Required disclosure "${tok}" is missing from the text.`);
    }
    if (sponsored) {
        for (const tok of tokenSet(disc.sponsored)) {
            if (present(tok))
                notes.push(`Sponsored disclosure "${tok}" present ✓`);
            else
                errors.push(`Sponsored post is missing the required disclosure "${tok}".`);
        }
    }
    const topics = tokenSet(p.banned_topics);
    if (topics.length)
        notes.push(`Banned topics — do not write about: ${topics.join(', ')} (agent-judged).`);
