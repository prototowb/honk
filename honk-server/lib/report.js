...
export function contentCheck(platform, content, account = '', { sponsored = false, scheduled_at = null, duplicateWindowHours = 168 } = {}) {
    ...
    const duplicate = dup
        ...
        : { hit: false };
    if (dup)
        warnings.push(`Possible duplicate — ${duplicate.detail} (window ${duplicateWindowHours}h). Intentional repost? Vary the copy (content-craft) or proceed deliberately.`);
...
const MARK = { pass: '✅ PASS', warn: '⚠ WARN', block: '✘ BLOCK' };
export function formatContentCheck(r) {
    const out = [`content_check — ${r.platform}: ${MARK[r.verdict]}`];
    if (r.errors.length)
        out.push('', 'Blocking errors (publish will refuse these):', ...r.errors.map(e => `  ✘ ${e}`));
    if (r.warnings.length)
        out.push('', 'Warnings:', ...r.warnings.map(w => `  ⚠ ${w}`));
    if (r.notes.length)
        out.push('', 'Notes:', ...r.notes.map(n => `  · ${n}`));
    out.push('', 'Agent-judged gates (confirm each — the server cannot check these):', ...r.agentGates.map(g => `  ☐ ${g}`));
    if (r.verdict === 'pass')
        out.push('', 'All deterministic checks passed. Work the checklist, then publish.');
    if (r.verdict === 'block')
        out.push('', 'Fix the errors first — the dispatch gate enforces them on every publish path.');
    return out.join('\n');
}
