...
<!-- AUTO-GENERATED by `npm run build` (build/generate.mjs) — DO NOT EDIT.
     Edit the prose source: capabilities/skills/post-to-threads.md   ·   Regenerate: npm run build   ·   Verify: npm run build:check -->

## Posting to Threads

Use `threads_post` from the `honk` MCP server.
...
- **Front-load the hook.** The 500-char cap rewards tightness; say the interesting thing first.
...
Max 500 characters for text. On image posts, add **`alt_text`** — an accessibility description of the image — by default.
