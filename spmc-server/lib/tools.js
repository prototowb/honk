...
export const TOOLS = [
  ...
  {
    name: 'brand_voice',
    description: 'Get or set the brand kit — a persistent profile (voice: tone, audience, hashtag sets, emoji/banned-word policy, CTA library, UTM rules; plus a visual identity block: accent/bg/surface/heading/body colors, logo, icon, handle, default template; plus per-platform voice deltas; plus named audience segments; plus a content policy: banned topics, required disclosures, auto-publish) that the content skills read so drafts match your voice, composed images match your look, and posts respect your guardrails without re-specifying it each time. Per account (omit account for the default). Content config, not secrets. Call with action:"get" first to see the current profile; if it is empty, offer guided setup (see brand_schema / the brand-setup skill). Pass a platform and/or audience with action:"get" to see the effective voice resolved for it (base merged with audience-segment, then per-platform, overrides — precedence base ▸ audience ▸ platform).',
    inputSchema: {
      type: 'object',
      properties: {
