...
export const TOOLS = [
  ...
  // ── Instagram ─────────────────────────────────────────────────────────────
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        alt_text:      { type: 'string', description: 'Accessibility alt text for a single-image post (describe the image for screen readers).' },
        alt_texts:     { type: 'array', items: { type: 'string' }, description: 'Per-slide alt text for a carousel — one entry per image_urls item, in order.' },
  ...
  // ── Facebook ──────────────────────────────────────────────────────────────
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        alt_text:      { type: 'string', description: 'Accessibility alt text for the attached photo (only applies when image_url is set).' },
  ...
  {
    name: 'brand_voice',
    description: 'Get or set the brand kit, and manage multiple brand accounts — a persistent profile (voice: tone, audience, hashtag sets, emoji/banned-word policy, CTA library, UTM rules; plus a visual identity block: accent/bg/surface/heading/body colors, logo, icon, handle, default template; plus per-platform voice deltas; plus named audience segments; plus a content policy: banned topics, required disclosures, auto-publish) that the content skills read so drafts match your voice, composed images match your look, and posts respect your guardrails without re-specifying it each time. Per account (omit account for the default). Content config, not secrets. Call with action:"get" first to see the current profile; if it is empty, offer guided setup (see brand_schema / the brand-setup skill). Pass a platform and/or audience with action:"get" to see the effective voice resolved for it (base merged with audience-segment, then per-platform, overrides — precedence base ▸ audience ▸ platform). Multi-brand: action:"list" enumerates accounts (brand profiles + credentialed accounts) and marks the active one; action:"use" sets the active account (reads default to it — get/brand_schema — but publishing stays explicit, so always confirm the brand before posting); action:"clone" copies a profile to a new account key (to:) as a starting point.',
    inputSchema: {
      type: 'object',
      properties: {
        action:  { type: 'string', description: 'get (default) reads the profile; set writes one; clear removes it; list enumerates accounts (marking the active one); use sets the active account; clone copies a profile to a new account (to:).', enum: ['get', 'set', 'clear', 'list', 'use', 'clone'] },
        profile: { type: 'object', description: 'For action:set — the fields to write, deep-merged into the stored profile (nested objects merge; arrays/scalars replace). Shape: { voice:{tone,audience,register,emoji_policy,banned_words[],do[],dont[]}, hashtags:{default[],sets{}}, cta[], visual:{accent,bg_color,surface,heading_color,body_color,logo_url,icon_url,handle,default_template}, links:{utm_defaults{},shortener}, platforms:{ <platform>:{tone,register,emoji_policy,audience,hashtags[],cta[]} }, audiences:{ <name>:{tone,register,emoji_policy,hashtags[],cta[]} }, policy:{banned_topics[],disclosures:{always[],sponsored[]},auto_publish}, notes }. The visual block is the brand identity media_compose defaults from; platforms holds per-platform voice deltas and audiences holds named audience segments (a set value replaces the base; a segment can NOT set audience — selecting it IS the audience); policy holds content guardrails — disclosures.always tokens warn when missing, disclosures.sponsored tokens error on a sponsored post, banned_topics are agent-judged reminders, auto_publish:false keeps the always-confirm rule.' },
        replace: { type: 'boolean', description: 'For action:set — overwrite the whole profile instead of deep-merging.' },
        account: { type: 'string', description: "Named account (e.g. 'brand'). For set/clear/clone it is the explicit target (omit = default). For use it is the account to activate (omit = reset to default). For get/brand_schema, omit to use the ACTIVE account (set via action:\"use\"), else the default." },
        to: { type: 'string', description: 'For action:clone — the new account key to copy the profile into (must not already have a profile). The source is `account` (omit = default).' },
        platform: { type: 'string', description: 'For action:get — resolve the effective voice for this platform (base merged with profile.platforms overrides), marking which fields each layer changed. Combine with audience. Omit to get the raw full profile.', enum: ['x', 'instagram', 'tiktok', 'facebook', 'threads', 'bluesky'] },
        audience: { type: 'string', description: 'For action:get — resolve a named audience segment (from profile.audiences) over the base voice; combine with platform (precedence base ▸ audience ▸ platform). A name with no matching segment falls back to the base and is flagged. Omit for no audience layer.' },
      },
    },
  },
  {
