...
export async function uploadCloudinary(filePathOrBuffer, account = '', filename) {
  ...
  const cloudName = process.env[`CLOUDINARY_CLOUD_NAME${sfx}`];
  const apiKey    = process.env[`CLOUDINARY_API_KEY${sfx}`];
  const apiSecret = process.env[`CLOUDINARY_API_SECRET${sfx}`];

  if (!cloudName || !apiKey || !apiSecret)
    throw new Error(`Cloudinary credentials missing${account ? ` for account "${account}"` : ''}. Set CLOUDINARY_CLOUD_NAME, CLOUDINARY_API_KEY, CLOUDINARY_API_SECRET.`);
...
export async function upload(filePathOrBuffer, provider = null, account = '', _buf, _filename) {
  ...
  const fname = _filename;

  const sfx = account ? `__${account.toUpperCase()}` : '';
  const hasCloudinary = !!(process.env[`CLOUDINARY_CLOUD_NAME${sfx}`] && process.env[`CLOUDINARY_API_KEY${sfx}`] && process.env[`CLOUDINARY_API_SECRET${sfx}`]);
  const hasImgbb      = !!process.env[`IMGBB_API_KEY${sfx}`];

  const selected = provider ?? (hasCloudinary ? 'cloudinary' : hasImgbb ? 'imgbb' : null);
  if (!selected)
    throw new Error('No media provider configured. Set CLOUDINARY_* or IMGBB_API_KEY in your env.');

  switch (selected) {
    case 'cloudinary': return uploadCloudinary(input, account, fname);
    case 'imgbb':      return uploadImgbb(input, account, fname);
    default: throw new Error(`Unknown provider: ${selected}. Supported: cloudinary, imgbb`);
  }
}
