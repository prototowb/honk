...
- **Image alt text** (ALPHA-014) on posts that carry an image — `alt_text` on
  ...
  through the queue. (IG via the `/media` `alt_text` field, FB via `alt_text_custom`,
  Threads via the container `alt_text` param; Threads is built but unverified live.)
- **First comment** (ALPHA-015) on Instagram and Facebook — `first_comment` posts a
  ...
  **Best-effort by design:** it runs *after* the publish is confirmed and audited,
  so a comment failure (e.g. a missing permission) never marks the live post failed
  or blinds `duplicate_check` — the outcome is folded into the result summary.
