...
(**INDIV-004 shipped — see Completed**; INDIV-005 is next):
| ID | Title | Notes |
|----|-------|-------|
| INDIV-005 | **Audience segments** — `audiences{}` second axis; generalize `PLATFORM_OVERRIDE_FIELDS`→`OVERRIDE_FIELDS`; `resolveVoice(profile,{platform,audience})`; `audience` brief field | Build **next**. Clean extension of INDIV-003. Confirm precedence (base ▸ audience ▸ platform) |
...
| Tests | 110 unit (`node:test`) + 33-check MCP smoke test |
